...
class Program
{
    delegate void ExceptionThrower(double a, double b);

    static double CalculateRoot(double a, double b, int exceptionType)
    {
        if (a == 0)
        {
            ExceptionThrower thrower = null;

            switch (exceptionType)
            {
                case 1:
                    thrower = (aVal, bVal) => { throw new LinearEquationError("Коэффициент 'a' равен 0."); };
                    break;
                case 2:
                    thrower = (aVal, bVal) => { throw new ArgumentException("Коэффициент 'a' не может быть равен нулю."); };
                    break;
                case 3:
                    thrower = (aVal, bVal) => { throw new LinearEquationException("Линейное уравнение не имеет решения (a = 0).", a, b); };
                    break;
                default:
                    thrower = (aVal, bVal) => { throw new ArgumentException("Неверный тип исключения."); };
                    break;
            }
            thrower(a, b);
        }
